...
class ShipStationProviderService extends AbstractFulfillmentProviderService {
  ...
  private getLineItemWeightGrams(
    item: CartLineItemDTO | OrderLineItemDTO
  ): number {
    const perUnit =
  ...
  private async createShipment({
    ...
    if (packageWeightGrams <= 0) {
      throw new MedusaError(
        MedusaError.Types.INVALID_DATA,
        "Cannot calculate shipping rates: cart items have no weight. Set weight (in grams) on each product or variant in the admin."
      )
    }

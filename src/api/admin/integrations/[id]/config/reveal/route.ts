...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  const body = (req.body ?? {}) as { reveal_key?: string }

  if (!body.reveal_key?.trim()) {
    throw new MedusaError(
      MedusaError.Types.INVALID_DATA,
      "reveal_key is required."
    )
  }
  ...
  const result = integrationsHealthService.revealConfig(
    id,
    body.reveal_key.trim()
  )

  res.json({ config_reveal: result })
}
